...
class ModifyElfCard extends ConsumerWidget {
  final ElfModel elf;

  const ModifyElfCard({super.key, required this.elf});
          ...
          Clickable(
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => ElfModifyPage(elf: elf),
                ),
              );
            },
            child: Card(
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(20),
                child: getElfImage(elf.id),
              ),
            ),
          ),
          Text(elf.name, style: TextStyle(fontSize: 16, color: Colors.white)),
        ],
      ),
    );
  }
